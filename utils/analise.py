...
from utils.funcoesAnalise import ANALISIS_FOLDER, DataAnalyzer
...
def run_complete_analysis(verbose: bool = True, tipo: str = None) -> dict:
    ...
    grouped_city = analyzer.group_by_location(by='cidade')
    print(f"✓ {len(grouped_city)} cidades analisadas")
    print("\nTop 5 cidades (por preço médio/m²):")
    print(grouped_city[['cidade', 'price_per_m2_mean', 'price_per_m2_median', 'price_per_m2_count']].head())
    print()

    print("5. Agrupando dados por bairro...")
    grouped_bairro = analyzer.group_by_location(by='bairro')
    print(f"✓ {len(grouped_bairro)} bairros analisados")
    print("\nTop 5 bairros (por preço médio/m²):")
    print(grouped_bairro[['bairro', 'price_per_m2_mean', 'price_per_m2_median', 'price_per_m2_count']].head())
    print()
    ...
    output_path = Path(ANALISIS_FOLDER)
    ...
    grouped_bairro.to_csv(output_path / 'agrupado_bairro.csv', index=False)
