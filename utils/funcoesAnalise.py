...
ANALISIS_FOLDER = os.path.join(PATH_OUTPUT, "analise")
FOLDER_GRAPHS = os.path.join(PATH_OUTPUT, "graficos")
...
class DataAnalyzer:
    ...
    def CriaVisualizacao(self,
                         ...
                         by: Literal['cidade', 'bairro'] = 'cidade',
                         output_path: str = FOLDER_GRAPHS,
    ...
    def criaTotalsVisualizacoes(self, output_path: str = FOLDER_GRAPHS) -> list[str]:
        saved_files = []
    ...
    def GerarRelatorio(self, output_path: str = ANALISIS_FOLDER) -> str:
        if self.dadosLimpos is None or 'price_per_m2' not in self.dadosLimpos.columns:
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            f.write("-" * 80 + "\n")
            top_cities = self.dadosLimpos.groupby('cidade')['price_per_m2'].agg(['mean', 'count']).sort_values('mean', ascending=False).head(10)
            ...
            f.write("-" * 80 + "\n")
            dados_temp = self.dadosLimpos.dropna(subset=['bairro'])
            top_bairros = dados_temp.groupby('bairro')['price_per_m2'].agg(['mean', 'count']).sort_values('mean', ascending=False).head(10)
            for i, (bairro, row) in enumerate(top_bairros.iterrows(), 1):
                f.write(f"{i:2d}. {bairro:30s} - R$ {row['mean']:8.2f}/m² ({int(row['count'])} imóveis)\n")
